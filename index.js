...
bot.on('messageReactionAdd', function (messageReaction, user) {
    ...
    if (!messageReaction.message.guild.member(user).hasPermission("ADMINISTRATOR")){ return;}
    if (messageReaction.message.channel.id != utility.configObj.outputChannel) {return; }
    if (messageReaction.message.member.id != "294470644526088192"){ return;} //TROCAR DEPOIS
    if (messageReaction.message.guild.member(user).id == "294470644526088192") { return; } //TROCAR DEPOIS
    if (messageReaction.message.embeds.length == 0){ return;}
