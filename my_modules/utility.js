...
exports = module.exports = {
    ...
    writeLog: function(string, logLevel)
    {
        ...
        {
            var type = "";
            if (logLevel == 1){type = "[ERROR]"} else if (logLevel == 2){type = "[WARN]"} else {type = "[INFO]"};
    ...
    {
        if (!fs.existsSync("./data/"+serverId)){return null;}
        if (!fs.existsSync("./data/" + serverId + "/" + memberId + ".json")){return null;}
        return (JSON.parse(fs.readFileSync(path.resolve(process.cwd(),"./data/" + serverId + "/" + memberId + ".json"))));
    },
    writeMemberData: function(serverId, memberId, data)
    {
        ...
        var stream = fs.createWriteStream(path.resolve(process.cwd(),"./data/" + serverId + "/" + memberId + ".json"));
    ...
    {
        if (!fs.existsSync(path.resolve(process.cwd(),"./data/" + serverId))) { return null; }
        if (!fs.existsSync(path.resolve(process.cwd(),"./data/" + serverId + "/data.json"))) { return null; }
        return (JSON.parse(fs.readFileSync(path.resolve(process.cwd(),"./data/" + serverId + "/data.json"))));
    },
    writeServerData: function(serverId, data)
    {
        ...
        var stream = fs.createWriteStream(path.resolve(process.cwd(),"./data/" + serverId + "/data.json"));
        if (typeof (data) == "string")
        {
            stream.write(data);
        }
            ...
            if (msgSplit[0]) {
                for (i = 0; i < exports.commands.length; ++i) {
                    if (exports.commands[i].command.toLowerCase() == msgSplit[0].toLowerCase()) {
                        ...
                        {
                            if (exports.checkMemberPermission(message.guild.id,message.member.id,"ADMINISTRATOR"))
                            {
    ...
    sendProgress: function(obj)
    {
        ...
        var channel = exports.bot.guilds.get(obj.serverId).channels.get(exports.configObj.outputChannel);
        var showcaseChannel = exports.bot.guilds.get(obj.serverId).channels.get(exports.configObj.showcaseChannel);
